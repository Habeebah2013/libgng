...
static void newWeights(svo_nnbp_t *nn, const bor_vec_t *out);
...
svo_nnbp_t *svoNNBPNew(const svo_nnbp_params_t *params)
{
    ...
    max = 0;
    nn->layers = BOR_ALLOC_ARR(svo_nnbp_layer_t, nn->layers_num);
...
const bor_vec_t *svoNNBPFeed(svo_nnbp_t *nn, const bor_vec_t *in)
{
    ...
    for (i = 1; i < nn->layers_num; i++){
        for (n = 0; n < nn->layers[i].size; n++){
            ...
            val = BOR_ZERO;
            if (nn->func == SVO_NNBP_SIGMOID){
                val = sigmoid(sum, nn->lambda);
            }else if (nn->func == SVO_NNBP_SIGMOID01){
...
{
    svo_nnbp_layer_t *lr = &nn->layers[nn->layers_num - 1];
...
static void makeDelta(svo_nnbp_t *nn, size_t layer,
                      const bor_vec_t *out,
                      const bor_vec_t *delta,
                      bor_vec_t *delta_out)
{
    svo_nnbp_layer_t *lr, *lr2;
    bor_real_t err;
    size_t i, j;
