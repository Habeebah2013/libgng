...
typedef struct _svo_gsrm_cache_t svo_gsrm_cache_t;


/** Allocates and deallocates cache */
static svo_gsrm_cache_t *cacheNew(void);
static void cacheDel(svo_gsrm_cache_t *c);


/** --- Node functions --- */
/** Creates new node and sets its weight to given vector */
static node_t *nodeNew(svo_gsrm_t *g, const bor_vec3_t *v);
/** Deletes node */
static void nodeDel(svo_gsrm_t *g, node_t *n);
...
_bor_inline void nodeFixError(svo_gsrm_t *gng, node_t *n);
/** Increment error counter */
_bor_inline void nodeIncError(svo_gsrm_t *gng, node_t *n, bor_real_t inc);
/** Scales error counter */
_bor_inline void nodeScaleError(svo_gsrm_t *gng, node_t *n, bor_real_t scale);


/** --- Edge functions --- */
/** Creates new edge as connection between two given nodes */
static edge_t *edgeNew(svo_gsrm_t *g, node_t *n1, node_t *n2);
/** Deletes edge */
static void edgeDel(svo_gsrm_t *g, edge_t *e);
/** Deteles edge - proposed for borMesh3Del2() function */
static void edgeDel2(bor_mesh3_edge_t *v, void *data);

/** --- Face functions --- */
static face_t *faceNew(svo_gsrm_t *g, edge_t *e, node_t *n);
static void faceDel(svo_gsrm_t *g, face_t *e);
static void faceDel2(bor_mesh3_face_t *v, void *data);


static int init(svo_gsrm_t *g);
static void adapt(svo_gsrm_t *g);
static void newNode(svo_gsrm_t *g);

/* Initializes mesh with three random nodes from input */
static void meshInit(svo_gsrm_t *g);

static void drawInputPoint(svo_gsrm_t *g);
/** Performes Extended Competitive Hebbian Learning */
static void echl(svo_gsrm_t *g);
static void echlConnectNodes(svo_gsrm_t *g);
static void echlMove(svo_gsrm_t *g);
static void echlUpdate(svo_gsrm_t *g);
/** Creates new node */
static void createNewNode(svo_gsrm_t *g);
...
static node_t *nodeWithHighestErrCounter(svo_gsrm_t *g);
/** Returns node with highests error counter that is neighbor of sq */
static node_t *nodesNeighborWithHighestErrCounter(svo_gsrm_t *g, node_t *sq);
/** Actually creates new node between sq and sf */
static node_t *createNewNode2(svo_gsrm_t *g, node_t *sq, node_t *sf);


/** --- Topology learning --- */
static void learnTopology(svo_gsrm_t *g);


/** --- Postprocessing functions --- */
/** Returns (via min, max, avg arguments) minimum, maximum and average area
 *  of faces in a mesh. */
static void faceAreaStat(svo_gsrm_t *g, bor_real_t *min, bor_real_t *max,
                         bor_real_t *avg);
/** Deletes incorrect faces from mesh */
static void delIncorrectFaces(svo_gsrm_t *g);
/** Deletes incorrect edges from mesh */
static void delIncorrectEdges(svo_gsrm_t *g);
/** Merges all edges that can be merged */
static void mergeEdges(svo_gsrm_t *g);
/** Tries to finish (triangulate) surface */
static void finishSurface(svo_gsrm_t *g);
/** Deletes lonely nodes, edges and faces */
static void delLonelyNodesEdgesFaces(svo_gsrm_t *g);
/** Embed triangles everywhere it can */
static void finishSurfaceEmbedTriangles(svo_gsrm_t *g);
/** Returns true if all internal angles of face is smaller than
 *  g->params.max_angle */
static int faceCheckAngle(svo_gsrm_t *g, bor_mesh3_vertex_t *v1,
...
static void delFacesDangle(svo_gsrm_t *g, face_t *f1, face_t *f2);
...
static int finishSurfaceTriangle(svo_gsrm_t *g, edge_t *e);
/** Tries to create completely new face */
static int finishSurfaceNewFace(svo_gsrm_t *g, edge_t *e);

static int errHeapLT(const bor_pairheap_node_t *_n1,
                     const bor_pairheap_node_t *_n2,
                     void *data);

void svoGSRMParamsInit(svo_gsrm_params_t *params)
{
    params->lambda = 200;
    params->eb = 0.05;
    params->en = 0.0006;
...
svo_gsrm_t *svoGSRMNew(const svo_gsrm_params_t *params)
{
    svo_gsrm_t *g;

    g = BOR_ALLOC(svo_gsrm_t);
    g->params = *params;
...
size_t svoGSRMAddInputSignals(svo_gsrm_t *g, const char *fn)
{
    return borPCAddFromFile(g->is, fn);
}

int svoGSRMRun(svo_gsrm_t *g)
{
    size_t cycle;
    ...
    do {
        ...
        if (g->params.verbosity >= 2
                && bor_unlikely(cycle == SVO_GSRM_PROGRESS_REFRESH)){
            PR_PROGRESS(g);
            cycle = 0;
        }
...
int svoGSRMPostprocess(svo_gsrm_t *g)
{
...
_bor_inline void nodeFixError(svo_gsrm_t *g, node_t *n)
{
    unsigned long diff;
...
_bor_inline void nodeIncError(svo_gsrm_t *g, node_t *n, bor_real_t inc)
{
    nodeFixError(g, n);
    n->err += inc;
...
_bor_inline void nodeScaleError(svo_gsrm_t *g, node_t *n, bor_real_t scale)
{
    nodeFixError(g, n);
    n->err *= scale;
    borPairHeapUpdate(g->err_heap, &n->err_heap);
...
static void faceDel(svo_gsrm_t *g, face_t *f)
{
...
/** --- ECHL functions --- **/
static void decreaseAllErrors(svo_gsrm_t *g)
{
...
/** --- Create New Node functions --- **/
static node_t *nodeWithHighestErrCounterLinear(svo_gsrm_t *g)
{
    bor_list_t *list, *item;
    bor_mesh3_vertex_t *v;
    node_t *n;
...
/** --- Postprocessing functions --- */
static void faceAreaStat(svo_gsrm_t *g, bor_real_t *_min, bor_real_t *_max,
                         bor_real_t *_avg)
{
