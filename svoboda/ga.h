/***
 * Svoboda
 * --------
 * Copyright (c)2011 Daniel Fiser <danfis@danfis.cz>
 *
 *  This file is part of Svoboda.
 *
 *  Distributed under the OSI-approved BSD License (the "License");
 *  see accompanying file BDS-LICENSE for details or see
 *  <http://www.opensource.org/licenses/bsd-license.php>.
 *
 *  This software is distributed WITHOUT ANY WARRANTY; without even the
 *  implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
 *  See the License for more information.
 */

#ifndef __SVO_GA_H__
#define __SVO_GA_H__

#include <pthread.h>
#include <boruvka/core.h>
#include <boruvka/rand-mt.h>

#ifdef __cplusplus
extern "C" {
#endif /* __cplusplus */

/**
 * Genetic Algorithm
 * ==================
 *
 */

struct _svo_ga_t;


/**
 * Operators
 * ----------
 *
 * See svo_ga_ops_t.
 */

/** vvvv */
/**
 * Returns true if algorithm should terminate.
 */
typedef int (*svo_ga_terminate)(struct _svo_ga_t *ga, void *);

/**
 * Evaluates genotype and puts a fitness values into {fitness}
 */
typedef void (*svo_ga_eval)(struct _svo_ga_t *ga, void *genotype,
                            bor_real_t *fitness, void *data);

/**
 * Initializes genotype
 */
typedef void (*svo_ga_init)(struct _svo_ga_t *ga, void *genotype, void *data);

/**
 * Returns index of selected individual from population
 */
typedef size_t (*svo_ga_sel)(struct _svo_ga_t *ga, void *data);

/**
 * Performs a crossover.
 * Note that {outg[i]} can be NULL and thus must be avoided.
 */
typedef void (*svo_ga_crossover)(struct _svo_ga_t *ga,
                                 void **ing, void **outg, void *data);

/**
 * Performs a mutation on genotype.
 */
typedef void (*svo_ga_mutate)(struct _svo_ga_t *ga, void *genotype, void *data);

/**
 * Fills {sel} array with IDs of selected individuals and returns number of
 * selected individuals. This number must be maximally
 * params.presel_max.
 */
typedef size_t (*svo_ga_presel)(struct _svo_ga_t *ga, size_t *sel, void *data);

/**
 * Callback that is peridically called from GNG.
 *
 * It is called every .callback_period'th added node.
 */
typedef void (*svo_ga_callback)(struct _svo_ga_t *ga, void *);
/** ^^^^ */

struct _svo_ga_ops_t {
    svo_ga_eval      eval;      /*!< Default: NULL */
    svo_ga_terminate terminate; /*!< Default: NULL */
    svo_ga_init      init;      /*!< Default: NULL */
    svo_ga_sel       sel;       /*!< Default: svoGASelTournament2 */
    svo_ga_crossover crossover; /*!< Default: svoGACrossover2 */
    svo_ga_mutate    mutate;    /*!< Default: svoGAMutateNone */
    svo_ga_presel    presel;    /*!< Default: NULL */

    svo_ga_callback callback;      /*!< Default: NULL */
    unsigned long callback_period; /*!< Default: 0, i.e. never */

    void *data; /*!< Data pointer that will be provided to all callbacks if
                     not specified otherwise. */

    void *eval_data;
    void *terminate_data;
    void *init_data;
    void *sel_data;
    void *crossover_data;
    void *mutate_data;
    void *presel_data;
    void *callback_data;
};
typedef struct _svo_ga_ops_t svo_ga_ops_t;


/**
 * Initialize ops to default values
 */
void svoGAOpsInit(svo_ga_ops_t *ops);


/**
 * Parameters
 * -----------
 */
struct _svo_ga_params_t {
    bor_real_t pc;         /*!< Probability of crossover. Default: 0.7 */
    bor_real_t pm;         /*!< Probability of mutation. Default: 0.001 */
    size_t gene_size;      /*!< Size of one gene (in bytes). Default: 1 */
    size_t genotype_size;  /*!< Number of genes in genotype. Default: 1 */
    size_t pop_size;       /*!< Size of population. Default: 1 */
    size_t fitness_size;   /*!< Number of fitness values. Default: 1 */
    size_t crossover_size; /*!< Number of individuals selected for crossover.
                                Default: 2 */
    size_t presel_max;     /*!< Maximal number of preselected individuals.
                                Default: 10 */
    int threads;           /*!< Number of threads. Default: 1 */
};
typedef struct _svo_ga_params_t svo_ga_params_t;


/**
 * Initialize params to default values
 */
void svoGAParamsInit(svo_ga_params_t *p);


/**
 * GA Algorithm
 * -------------
 */
struct _svo_ga_t {
    svo_ga_params_t params;
    svo_ga_ops_t ops;

    void **pop[2];   /*!< Population */
    size_t pop_cur; /*!< Index of current population */
    void **gt[2];
    bor_real_t **ft[2];
    size_t *presel;

    bor_rand_mt_t *rand;


    int tid; /*!< Thread ID, in case threads are used */
    int tfrom, tto;
    pthread_mutex_t *tlock;
    bor_real_t *trand;
    size_t trand_next, trand_max;
};
typedef struct _svo_ga_t svo_ga_t;


/**
 * Creates new instance of GA
 */
svo_ga_t *svoGANew(const svo_ga_ops_t *ops, const svo_ga_params_t *params);

/**
 * Deletes GA
 */
void svoGADel(svo_ga_t *ga);

/**
 * Runs genetic algorithm
 */
void svoGARun(svo_ga_t *ga);

/**
 * Returns random number from range
 */
_bor_inline bor_real_t svoGARand(svo_ga_t *ga, bor_real_t f, bor_real_t t);

/**
 * Returns random number [0, 1)
 */
_bor_inline bor_real_t svoGARand01(svo_ga_t *ga);

/**
 * Returns random integer number [f, t)
 */
_bor_inline int svoGARandInt(svo_ga_t *ga, int f, int t);

/**
 * Returns crossover probability
 */
_bor_inline bor_real_t svoGAParamPc(const svo_ga_t *ga);

/**
 * Returns mutation probability
 */
_bor_inline bor_real_t svoGAParamPm(const svo_ga_t *ga);

/**
 * Returns gene size (in bytes).
 */
_bor_inline size_t svoGAParamGeneSize(const svo_ga_t *ga);

/**
 * Returns genotype size, i.e., number of genes per genotype.
 */
_bor_inline size_t svoGAParamGenotypeSize(const svo_ga_t *ga);

/**
 * Returns population size
 */
_bor_inline size_t svoGAParamPopSize(const svo_ga_t *ga);

/**
 * Returns fitness size, i.e., number of fitness values.
 */
_bor_inline size_t svoGAParamFitnessSize(const svo_ga_t *ga);

/**
 * Returns crossover size, i.e., number of individuals that undergo a
 * crossover operator.
 */
_bor_inline size_t svoGAParamCrossoverSize(const svo_ga_t *ga);



/**
 * Returns i'th individual
 */
_bor_inline void *svoGAIndiv(svo_ga_t *ga, size_t i);

/**
 * Returns pointer to fitness array of the individual
 */
_bor_inline bor_real_t *svoGAIndivFitness(svo_ga_t *ga, void *indiv);

/**
 * Returns pointer to genotype array of the individual
 */
_bor_inline void *svoGAIndivGenotype(svo_ga_t *ga, void *indiv);

/**
 * Returns pointer to specific gene of the individual
 */
_bor_inline void *svoGAIndivGene(svo_ga_t *ga, void *indiv, size_t genepos);




/**
 * Tournament selection of two individuals
 */
size_t svoGASelTournament2(svo_ga_t *ga, void *data);

/**
 * Tournament selection of three individuals
 */
size_t svoGASelTournament3(svo_ga_t *ga, void *data);

/**
 * Crossover operator between two individuals
 */
void svoGACrossover2(svo_ga_t *ga, void **ing, void **outg, void *data);


/**
 * No mutation
 */
void svoGAMutateNone(svo_ga_t *ga, void *gt, void *data);


/**
 * Preselect operator that selects params.presel_max best individuals
 * (based on first fitness value)
 */
size_t svoGAPreselElite(svo_ga_t *ga, size_t *sel, void *data);



void __svoGATRandRefill(svo_ga_t *ga);

/**** INLINES ****/
_bor_inline bor_real_t svoGARand(svo_ga_t *ga, bor_real_t f, bor_real_t t)
{
    if (ga->tid == -1)
        return borRandMT(ga->rand, f, t);

    if (ga->trand_next == ga->trand_max)
        __svoGATRandRefill(ga);
    return (ga->trand[ga->trand_next++] * (t - f)) + f;
}

_bor_inline bor_real_t svoGARand01(svo_ga_t *ga)
{
    if (ga->tid == -1)
        return borRandMT01(ga->rand);

    if (ga->trand_next == ga->trand_max)
        __svoGATRandRefill(ga);
    return ga->trand[ga->trand_next++];
}

_bor_inline int svoGARandInt(svo_ga_t *ga, int f, int t)
{
    int v;
    v = svoGARand(ga, f, t);
    return (v < t ? v : t - 1);
}

_bor_inline bor_real_t svoGAParamPc(const svo_ga_t *ga)
{
    return ga->params.pc;
}

_bor_inline bor_real_t svoGAParamPm(const svo_ga_t *ga)
{
    return ga->params.pm;
}

_bor_inline size_t svoGAParamGeneSize(const svo_ga_t *ga)
{
    return ga->params.gene_size;
}

_bor_inline size_t svoGAParamGenotypeSize(const svo_ga_t *ga)
{
    return ga->params.genotype_size;
}

_bor_inline size_t svoGAParamPopSize(const svo_ga_t *ga)
{
    return ga->params.pop_size;
}

_bor_inline size_t svoGAParamFitnessSize(const svo_ga_t *ga)
{
    return ga->params.fitness_size;
}

_bor_inline size_t svoGAParamCrossoverSize(const svo_ga_t *ga)
{
    return ga->params.crossover_size;
}


_bor_inline void *svoGAIndiv(svo_ga_t *ga, size_t i)
{
    return ga->pop[ga->pop_cur][i];
}

_bor_inline bor_real_t *svoGAIndivFitness(svo_ga_t *ga, void *indiv)
{
    return (bor_real_t *)indiv;
}

_bor_inline void *svoGAIndivGenotype(svo_ga_t *ga, void *indiv)
{
    char *ind;
    size_t fitness_size;

    fitness_size = sizeof(bor_real_t) * ga->params.fitness_size;
    ind = ((char *)indiv) + fitness_size;

    return (void *)ind;
}

_bor_inline void *svoGAIndivGene(svo_ga_t *ga, void *indiv, size_t genepos)
{
    char *ind;

    ind = (char *)svoGAIndivGenotype(ga, indiv);
    ind = ind + ga->params.gene_size * genepos;

    return (void *)ind;
}

#ifdef __cplusplus
} /* extern "C" */
#endif /* __cplusplus */

#endif /* __SVO_GA_H__ */

